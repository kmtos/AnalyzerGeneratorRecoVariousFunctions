// -*- C++ -*-
//
// Package:    FakeRateMiniAODEstimateRegionA
// Class:      FakeRateMiniAODEstimateRegionA
// 
/**\class FakeRateMiniAODEstimateRegionA FakeRateMiniAODEstimateRegionA.cc Analyzer/src/FakeRateMiniAODEstimateRegionA.cc
...
class FakeRateMiniAODEstimateRegionA : public edm::EDAnalyzer {
   public:
      typedef reco::JetFloatAssociation::Container JetBCEnergyRatioCollection;
      explicit FakeRateMiniAODEstimateRegionA(const edm::ParameterSet&);
      ~FakeRateMiniAODEstimateRegionA();
...
//
FakeRateMiniAODEstimateRegionA::FakeRateMiniAODEstimateRegionA(const edm::ParameterSet& iConfig):
...
FakeRateMiniAODEstimateRegionA::~FakeRateMiniAODEstimateRegionA()
{
  reset(true);
}
...
void FakeRateMiniAODEstimateRegionA::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
...
//////////////////////////////
  ...
  double bestMu3dR = 10000000, tauPt = -1, tauEta = -1000000;
...
std::cout << "check7" << std::endl;
  TH2F *histFR_ = (TH2F*)TH2File->Get(HistName_.c_str() );

  for (int i=1; i < histFR_->GetXaxis()->GetNbins(); i++)
  {
    for (int j=1; j < histFR_->GetYaxis()->GetNbins(); j++)
      std::cout << "histFR_->GetBinContent(" << i << "," << j << ")=" << histFR_->GetBinContent(i,j) << std::endl;
  }//for i
//  TCanvas* histFRCanvas = (TCanvas*)TH2File->Get(HistName_.c_str() );
//std::cout << "check8" << std::endl;
//  TH2F* histFR_ = (TH2F*)histFRCanvas->GetPrimitive(HistName_.c_str());
//std::cout << "check9" << std::endl;
  TAxis *xaxis = histFR_->GetXaxis();
std::cout << "check10" << std::endl;
  TAxis *yaxis = histFR_->GetYaxis();
std::cout << "check11" << std::endl;
  Int_t binx = xaxis->FindBin(tauPt  );
std::cout << "check12" << std::endl;
  Int_t biny = yaxis->FindBin(tauEta );
  double rate = histFR_->GetBinContent(binx, biny);
  std::cout << "rate= " << rate << "\tbinx=" <<  binx << "\tbiny= " << biny << std::endl;
  while (rate == 0 && binx > 0)
  {
  ...
  double fakeRateWeight = rate / (1 - rate);
std::cout << "check7" << std::endl;

  DiMuInvMassFakeWeight_->Fill(diMuP4.M(),  fakeRateWeight);
  DiMuDiTauInvMassFakeWeight_->Fill(diMuDiTauP4.M(),  fakeRateWeight);
  DiTauInvMassFakeWeight_->Fill(diTauP4.M(),  fakeRateWeight);
  DiMuInvMassFakeWeightZoom_->Fill(diMuP4.M(),  fakeRateWeight);
  TauVisMass_->Fill(diTauP4.M(),  fakeRateWeight);
  ...
  DRFakeWeight_->Fill(dR_Mu1Mu2, fakeRateWeight);
  DRNoWeighting_->Fill(dR_Mu1Mu2);
}//End FakeRateMiniAODEstimateRegionA::analyze


// ------------ method called once each job just before starting event loop  ------------
void FakeRateMiniAODEstimateRegionA::beginJob()
{
...
void FakeRateMiniAODEstimateRegionA::endJob()
{
  //Make the Canvases
...
void FakeRateMiniAODEstimateRegionA::beginRun(edm::Run const&, edm::EventSetup const&) {}

// ------------ method called when ending the processing of a run  ------------
void FakeRateMiniAODEstimateRegionA::endRun(edm::Run const&, edm::EventSetup const&) {}

// ------------ method called when starting to processes a luminosity block  ------------
void FakeRateMiniAODEstimateRegionA::beginLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&) {}

// ------------ method called when ending the processing of a luminosity block  ------------
void FakeRateMiniAODEstimateRegionA::endLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&) {}

//Delete Memory
void FakeRateMiniAODEstimateRegionA::reset(const bool doDelete)
{
...
void FakeRateMiniAODEstimateRegionA::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
DEFINE_FWK_MODULE(FakeRateMiniAODEstimateRegionA);

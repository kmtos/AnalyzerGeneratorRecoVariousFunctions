// -*- C++ -*-
//
// Package:    FakeRateMiniAODGetRatesALLISO
// Class:      FakeRateMiniAODGetRatesALLISO
// 
/**\class FakeRateMiniAODGetRatesALLISO FakeRateMiniAODGetRatesALLISO.cc Analyzer/src/FakeRateMiniAODGetRatesALLISO.cc
...
class FakeRateMiniAODGetRatesALLISO : public edm::EDAnalyzer {
   public:
      typedef reco::JetFloatAssociation::Container JetBCEnergyRatioCollection;
      explicit FakeRateMiniAODGetRatesALLISO(const edm::ParameterSet&);
      ~FakeRateMiniAODGetRatesALLISO();
...
//
FakeRateMiniAODGetRatesALLISO::FakeRateMiniAODGetRatesALLISO(const edm::ParameterSet& iConfig):
...
FakeRateMiniAODGetRatesALLISO::~FakeRateMiniAODGetRatesALLISO()
{
  reset(true);
}
...
void FakeRateMiniAODGetRatesALLISO::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  for (edm::View<pat::Tau>::const_iterator iTau = pTaus->begin(); iTau != pTaus->end(); ++iTau)
  {
    ...
    for (edm::View<pat::Muon>::const_iterator iMu = pMuons->begin(); iMu != pMuons->end(); ++iMu)
    {
      ...
      if (currdR < mu3dRMax_ && currdR > mu3dRMin_ && currdR < bestMu3dR)
      {
        ...
        {
          std::cout << "JPsi Eff" << std::endl;
std::cout << "mu3.pt()=" << mu3.pt() << std::endl;
          double binyIDs_BF = IDsWeight_LowPt_ID->GetXaxis()->FindBin(mu3.pt());
          double binxIDs_BF = IDsWeight_LowPt_ID->GetYaxis()->FindBin(fabs(mu3.eta()));
...
}//End FakeRateMiniAODGetRatesALLISO::analyze


// ------------ method called once each job just before starting event loop  ------------
void FakeRateMiniAODGetRatesALLISO::beginJob()
{
...
void FakeRateMiniAODGetRatesALLISO::endJob()
{
  //Make the Canvases
...
void FakeRateMiniAODGetRatesALLISO::beginRun(edm::Run const&, edm::EventSetup const&) {}

// ------------ method called when ending the processing of a run  ------------
void FakeRateMiniAODGetRatesALLISO::endRun(edm::Run const&, edm::EventSetup const&) {}

// ------------ method called when starting to processes a luminosity block  ------------
void FakeRateMiniAODGetRatesALLISO::beginLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&) {}

// ------------ method called when ending the processing of a luminosity block  ------------
void FakeRateMiniAODGetRatesALLISO::endLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&) {}

//Delete Memory
void FakeRateMiniAODGetRatesALLISO::reset(const bool doDelete)
{
...
void FakeRateMiniAODGetRatesALLISO::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
DEFINE_FWK_MODULE(FakeRateMiniAODGetRatesALLISO);

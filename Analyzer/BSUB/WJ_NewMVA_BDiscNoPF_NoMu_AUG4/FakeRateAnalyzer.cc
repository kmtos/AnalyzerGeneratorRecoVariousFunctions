...
class FakeRateAnalyzer : public edm::EDAnalyzer {
   ...
   private:
      ...
      TH1F* NMuRemoved_;
      TH1F* TauMuTauHaddR_;
      TH1F* MatchedTauMuPtSignal_;
      TH1F* MatchedTauHadPtSignal_;
      TH1F* MatchedTauHadEtaSignal_;
      TH1F* MatchedHTSignal_;
      TH1F* MatchedBDiscCSVSignal_;
...
std::cout << "removedMu= " << removedMu << "  abs(iTauCJ->eta() )= " << abs(iTauCJ->eta() ) << " Pt_GenMatchCJ= " << Pt_GenMatchCJ << std::endl;
    ...
    {
      ///////////////////////////////////////
      // Finding if Gen Matched to Signal
      ///////////////////////////////////////
      bool signalGenMatched = false;
      double dRGMsig = 1000000000000000;
      int signalPdgIdGM = genMatchPDGIDTag_;
      for (reco::GenParticleCollection::const_iterator iGenParticle = pGenParts->begin(); iGenParticle != pGenParts->end(); ++iGenParticle)
      {
        if (abs(iGenParticle->pdgId() ) == genMatchPDGIDTag_)
        {
          for (unsigned int iChild = 0; iChild < iGenParticle->numberOfDaughters(); ++iChild)
	  {
	    reco::GenParticleRef childRef = iGenParticle->daughterRef(iChild);
            std::cout << "\tMom= " << iGenParticle->pdgId() << "\tchild= " << childRef->pdgId() << std::endl;
	    if (abs(childRef->pdgId() ) == 24)
	    {
              for (unsigned int iChild2 = 0; iChild2 < childRef->numberOfDaughters(); ++iChild2)
              {
                ...
                std::cout << "\tMom= " << iGenParticle->pdgId() << "\tchild= " << childRef->pdgId() << "\tgrandChild= " << grandChildRef->pdgId() << std::endl;
		double dPhiGM = reco::deltaPhi(iTauCJ->phi(), grandChildRef->phi() ), dEtaGM = iTauCJ->eta() - grandChildRef->eta(), dRGMTemp = sqrt( dEtaGM*dEtaGM + dPhiGM*dPhiGM );
	 	std::cout << "\t\tdRGMTemp= " << dRGMTemp << std::endl;
                if (dRGMTemp < .05 && dRGMsig > dRGMTemp)
                {
		  std::cout << "GenMatch: pdgId() =" << grandChildRef->pdgId() << "\tdRGMsig= " << dRGMTemp << std::endl;
                  dRGMsig = dRGMTemp;
                  signalGenMatched = true;
                  signalPdgIdGM = abs(grandChildRef->pdgId() );
                }//if drGM < .05
	      }//for iChild2
	    }//if pdgId == 24
	    else
	    {  
	      ...
	      std::cout << "\t\tdRGMTemp= " << dRGMTemp << std::endl;
  	      if (dRGMTemp < .05 && dRGMsig > dRGMTemp)
	      {
                std::cout << "GenMatch: pdgId() =" << childRef->pdgId() << "\tdRGM= " << dRGMTemp << std::endl;
	        dRGMsig = dRGMTemp;
	        signalGenMatched = true;
	        signalPdgIdGM = abs(childRef->pdgId() );
	      }//if drGM < .05
	    }//else
	  }//for iChild
        }// if signalGenMatched
      ...
      // Finding if Gen Matched to Signal
      ///////////////////////////////////////
      bool jetGenMatched = false;
      double dRGMjet = 1000000000000000;
      int jetPdgIdGM = genMatchPDGIDTag_;
      ...
      if (signalGenMatched && (!jetGenMatched || dRGMsig < dRGMjet) )  
      {
	// Filling Fake Gen Match PdgID
	std::cout << "SIG: Final pdgID()= " << signalPdgIdGM << std::endl;
	GenMatchPDGID_->Fill(signalPdgIdGM );
	if (signalPdgIdGM <= 6)
	  GenMatchType_->Fill(3);
	if (signalPdgIdGM == 15 || signalPdgIdGM == 16)
	  GenMatchType_->Fill(0);
	if (signalPdgIdGM == 13 || signalPdgIdGM == 14)
	  GenMatchType_->Fill(1);  
	if (signalPdgIdGM == 11 || signalPdgIdGM == 12)
	  GenMatchType_->Fill(2);

        ///////////////////////////////////////
        // Filling Cut Option Histograms
        ///////////////////////////////////////
        MatchedTauHadEtaSignal_->Fill(iTauCJ->eta() );
        MatchedTauHadPtSignal_->Fill(iTauCJ->pt() );
        MatchedTauMuPtSignal_->Fill(removedMuonRefs[0]->pt() );
        NEventsCuts_->Fill(0);
        double sumHT = 0;
        ...
        for (unsigned int iBTagInfo = 0; iBTagInfo != pCSV->size(); ++iBTagInfo)
        {
          if (reco::PFJetRef(pOldJets, iBTagInfo).key() == ((*iTauCJ).jetRef()).key() )
          {
            ...
            MatchedBDiscCSVSignal_->Fill(BDisc );
          }//if
        }//for iBTagInfo

        // Filling nEvents for passing specific cuts
        if (Pt_GenMatchCJ < 200)
          NEventsCuts_->Fill(1);
        if (removedMuonRefs[0]->pt() < 100)
          NEventsCuts_->Fill(2);
        if (abs(iTauCJ->eta() ) < 1.75 );
          NEventsCuts_->Fill(3);
        if (sumHT < 300)
          NEventsCuts_->Fill(4);
        if (BDisc < .5 )
      ...
      else if (jetGenMatched && (!signalGenMatched || dRGMsig > dRGMjet) )    
      {
        // Filling Fake Gen Match PdgID
        std::cout << "JET: Final pdgID()= " << jetPdgIdGM << std::endl;
      ...
      if (jetGenMatched && !signalGenMatched)
        NEventsCuts_->Fill(11);
      if (signalGenMatched && !jetGenMatched)
...
void FakeRateAnalyzer::beginJob()
{
  ...
  MatchedTauMuPtSignal_       = new TH1F("MatchedTauMuPtSignal", "", 200, 0, 500);
  MatchedTauHadPtSignal_       = new TH1F("MatchedTauHadPtSignal" , "", 200, 0, 500);
  MatchedTauHadEtaSignal_       = new TH1F("MatchedTauHadEtaSignal", "", 100, -2.5, 2.5);
  MatchedHTSignal_       = new TH1F("MatchedHTSignal", "", 100, 0, 1000);
  MatchedBDiscCSVSignal_       = new TH1F("MatchedBDiscCSVSignal", "", 100, 0, 1);
  MatchedTauMuPtJet_       = new TH1F("MatchedTauMuPtJet", "", 200, 0, 500);
  MatchedTauHadPtJet_       = new TH1F("MatchedTauHadPtJet", "", 200, 0, 500);
  MatchedTauHadEtaJet_       = new TH1F("MatchedTauHadEtaJet", "", 100, -2.5, 2.5);
  MatchedHTJet_       = new TH1F("MatchedHTJet", "", 100, 0, 1000);
  MatchedBDiscCSVJet_       = new TH1F("MatchedBDiscCSVJet", "", 100, 0, 1);
  NEventsCuts_       = new TH1F("NEventsCuts", "", 14, -.5, 13.5);
      NEventsCuts_->GetXaxis()->SetBinLabel(1, "TotalEvents");
      NEventsCuts_->GetXaxis()->SetBinLabel(2, "S: p_{T}(#tau)< 200");
      NEventsCuts_->GetXaxis()->SetBinLabel(3, "S: p_{T}(#mu) < 100");
      NEventsCuts_->GetXaxis()->SetBinLabel(4, "S: #eta < 1.75");
      NEventsCuts_->GetXaxis()->SetBinLabel(5, "S: HT < 300");
      NEventsCuts_->GetXaxis()->SetBinLabel(6, "S: BDisc < .5");
      NEventsCuts_->GetXaxis()->SetBinLabel(7, "J: p_{T}(#tau) < 200");
      NEventsCuts_->GetXaxis()->SetBinLabel(8, "J: p_{T}(#mu) < 100");
      NEventsCuts_->GetXaxis()->SetBinLabel(9, "J: #eta < 1.75"); 
      NEventsCuts_->GetXaxis()->SetBinLabel(10, "J: HT < 300");
      NEventsCuts_->GetXaxis()->SetBinLabel(11, "J: BDisc < .5");
      NEventsCuts_->GetXaxis()->SetBinLabel(12, "Jet GM");
      NEventsCuts_->GetXaxis()->SetBinLabel(13, "Sig GM");
      NEventsCuts_->GetXaxis()->SetBinLabel(14, "Jet and Sig");
...
void FakeRateAnalyzer::endJob()
{
  ...
  TCanvas MatchedTauMuPtSignalCanvas("MatchedTauMuPtSignal","",600,600);
  TCanvas MatchedTauHadPtSignalCanvas("MatchedTauHadPtSignal","",600,600);
  TCanvas MatchedTauHadEtaSignalCanvas("MatchedTauHadEtaSignal","",600,600);
  TCanvas MatchedHTSignalCanvas("MatchedHTSignal","",600,600); 
  TCanvas MatchedBDiscCSVSignalCanvas("MatchedBDiscCSVSignal","",600,600);
...
std::cout << "<----------------Declared Canvases-------------->" << std::endl;
  ...
  VariousFunctions::formatAndDrawCanvasAndHist1D(MatchedTauMuPtSignalCanvas, MatchedTauMuPtSignal_,
         1, 0, 0, kBlack, 7, 20, "Signal: p_{T}(#tau_{mu})", .04, .04, 1.1,  "", .04, .04, 1.0, false);
  VariousFunctions::formatAndDrawCanvasAndHist1D(MatchedTauHadPtSignalCanvas, MatchedTauHadPtSignal_,
         1, 0, 0, kBlack, 7, 20, "Signal: p_{T}(#tau_{H})", .04, .04, 1.1,  "", .04, .04, 1.0, false);
  VariousFunctions::formatAndDrawCanvasAndHist1D(MatchedTauHadEtaSignalCanvas, MatchedTauHadEtaSignal_,
         1, 0, 0, kBlack, 7, 20, "Signal: #eta(#tau_{H})", .04, .04, 1.1,  "", .04, .04, 1.0, false);
  VariousFunctions::formatAndDrawCanvasAndHist1D(MatchedHTSignalCanvas, MatchedHTSignal_,
         1, 0, 0, kBlack, 7, 20, "Signal: H_{T} of ak4PFJet with p_{T} > 20", .04, .04, 1.1,  "", .04, .04, 1.0, false);
  VariousFunctions::formatAndDrawCanvasAndHist1D(MatchedBDiscCSVSignalCanvas, MatchedBDiscCSVSignal_,
         1, 0, 0, kBlack, 7, 20, "Signal: CSV B Discriminant Value", .04, .04, 1.1,  "", .04, .04, 1.0, false);
...
std::cout << "<----------------Formatted Canvases and Histos-------------->" << std::endl;
  ...
  MatchedTauMuPtSignalCanvas.Write();
  MatchedTauHadPtSignalCanvas.Write();
  MatchedTauHadEtaSignalCanvas.Write();
  MatchedHTSignalCanvas.Write();
  MatchedBDiscCSVSignalCanvas.Write();
...
void FakeRateAnalyzer::reset(const bool doDelete)
{
  ...
  TauMuTauHaddR_ = NULL;
  if ((doDelete) && (MatchedTauMuPtSignal_ != NULL)) delete MatchedTauMuPtSignal_;
  MatchedTauMuPtSignal_ = NULL;
  if ((doDelete) && (MatchedTauHadPtSignal_ != NULL)) delete MatchedTauHadPtSignal_;
  MatchedTauHadPtSignal_ = NULL;
  if ((doDelete) && (MatchedTauHadEtaSignal_ != NULL)) delete MatchedTauHadEtaSignal_;
  MatchedTauHadEtaSignal_ = NULL;
  if ((doDelete) && (MatchedHTSignal_ != NULL)) delete MatchedHTSignal_;
  MatchedHTSignal_ = NULL;
  if ((doDelete) && (MatchedBDiscCSVSignal_ != NULL)) delete MatchedBDiscCSVSignal_;
  MatchedBDiscCSVSignal_ = NULL;

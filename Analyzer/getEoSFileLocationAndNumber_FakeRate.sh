...
mv FILE_TESTS/InputForFakeRateForB.txt ~/Copies_Temp_etc/
...
mv FILE_TESTS/InputForFakeRateForC.txt ~/Copies_Temp_etc/
...
mv FILE_TESTS/InputForFakeRate.txt ~/Copies_Temp_etc/

for i in /eos/cms/store/group/phys_higgs/HiggsExo/ktos/SingleMuon/*
do
  dirLs="$(echo $i | grep _TauDM_ | grep FEB8)"
  if [ -z "$dirLs" ]; then
    echo "Skipped $i"
    continue
  fi
  ...
  do
    ...
    echo $fileName
    dir="$(echo ${line##*/})"
    echo "$count  $dir  $fileName" | grep _TauDM_ | grep FEB8 >> FILE_TESTS/InputForFakeRate.txt
    done
done
